...
| `spark.default.parallelism` | 对于分布式混洗（shuffle）操作，如 `reduceByKey` 和 `join`，父 RDD 中分区的最大数量。对于没有父 RDD 的 `parallelize` 操作，它取决于集群管理器：<br>*   本地模式：本地机器上的 core 数<br>*   Mesos 细粒度模式：8<br>*   其他：所有执行器节点上的 core 总数或者 2，以较大者为准 | 如果用户没有指定参数值，则这个属性是 `join`，`reduceByKey`，和 `parallelize` 等转换返回的 RDD 中的默认分区数。 |
